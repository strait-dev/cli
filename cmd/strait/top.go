		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return runTopLoop(cmd, watch, interval, func() error {
				...
				for _, run := range runs {
					...
					if run.Status == types.StatusFailed || run.Status == types.StatusTimedOut || run.Status == types.StatusCrashed || run.Status == types.StatusSystemFailed {
						totals[run.JobID]["failed"]++
					}
					if run.Status == types.StatusQueued || run.Status == types.StatusDelayed || run.Status == types.StatusDequeued || run.Status == types.StatusExecuting || run.Status == types.StatusWaiting {
