...
import (
	...
	clitui "github.com/strait-dev/cli/internal/tui"
...
func tviewStatusColor(status types.RunStatus) string {
	...
	case types.StatusFailed, types.StatusSystemFailed, types.StatusCrashed:
		return "[red]" + string(status) + "[-]"
	case types.StatusExecuting, types.StatusQueued, types.StatusDequeued:
		return "[yellow]" + string(status) + "[-]"
	case types.StatusDelayed, types.StatusWaiting:
		return "[blue]" + string(status) + "[-]"
	case types.StatusCanceled, types.StatusExpired, types.StatusTimedOut:
