...
import (
	...
	"github.com/strait-dev/cli/internal/bundle"
...
func newProjectExportCommand(state *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			stepsMap := make(map[string][]types.WorkflowStep, len(workflows))
...
func newProjectImportCommand(state *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			existingJobsBySlug := make(map[string]types.Job, len(existingJobs))
			...
			existingWFsBySlug := make(map[string]types.Workflow, len(existingWorkflows))
...
func applyCreate(ctx context.Context, cli *client.Client, projectID string, entry bundle.DiffEntry, b *bundle.Bundle, existingJobs map[string]types.Job) error {
...
func applyUpdate(ctx context.Context, cli *client.Client, entry bundle.DiffEntry, b *bundle.Bundle, existingJobs map[string]types.Job, existingWFs map[string]types.Workflow) error {
	switch entry.ResourceType {
	case "job":
		spec := findJobSpec(b, entry.Slug)
		if spec == nil {
...
func buildWorkflowSteps(specSteps []bundle.WorkflowStepSpec, existingJobs map[string]types.Job) []client.WorkflowStepRequest {
