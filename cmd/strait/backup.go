...
func newBackupCommand(state *appState) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "backup",
		Short: "Backup and restore the strait database",
		Long:  "Wraps pg_dump and pg_restore for strait database backup and restore operations.",
	}
...
func newBackupCreateCommand(state *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Creates a backup of the strait database using pg_dump.

Requires pg_dump to be installed and available in PATH.`,
		Example: `  strait backup create
  ...
  strait backup create --format custom --output backup.dump`,
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if output == "" {
				output = fmt.Sprintf("strait-backup-%s.sql", time.Now().UTC().Format("20060102-150405"))
				if format == "custom" {
					output = fmt.Sprintf("strait-backup-%s.dump", time.Now().UTC().Format("20060102-150405"))
				}
			}
...
func newBackupRestoreCommand(state *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Restores an strait database from a backup file.
...
Requires pg_restore and/or psql to be installed and available in PATH.`,
		Example: `  strait backup restore --input backup.sql
  strait backup restore --input backup.dump --clean
  strait backup restore --input backup.dump --database-url postgres://user:pass@host:5432/db`,
		RunE: func(_ *cobra.Command, _ []string) error {
			if input == "" {
