...
func TestDoctorCommand_OutputIncludesBaselineChecks(t *testing.T) {
	t.Parallel()
	...
	if !strings.Contains(out, "connectivity_health") {
		t.Fatalf("expected connectivity_health in output, got: %s", out)
	}
	if !strings.Contains(out, "auth_stats") {
		t.Fatalf("expected auth_stats in output, got: %s", out)
	}
}
