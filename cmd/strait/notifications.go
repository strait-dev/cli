...
func newNotificationsCommand(state *appState) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "notifications",
		Aliases: []string{"notification-channels", "notification-channel"},
...
func newNotificationsListCommand(state *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			channels, err := cli.ListNotificationChannels(cmd.Context(), pid)
			if err != nil {
				return err
			}
			...
			if isTTYRich(state) {
				...
				for _, ch := range channels {
					fmt.Fprintf(os.Stderr, "  %s  %-20s  type=%s  %s\n",
						styles.Enabled(ch.Enabled),
						styles.Bold.Render(styles.SafeText(ch.Name)),
						styles.SafeText(ch.Type),
						styles.MutedStyle.Render(styles.SafeText(ch.ID)),
					)
				}
				return nil
			}
...
func newNotificationsGetCommand(state *appState) *cobra.Command {
	...
	cmd.Flags().BoolVar(&reveal, "reveal", false, "show channel config in plaintext")
	return cmd
}
...
func newNotificationsCreateCommand(state *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if isTTYRich(state) {
				fmt.Fprintln(os.Stderr, styles.Success("Created notification channel "+styles.Bold.Render(styles.SafeText(channel.Name))))
				return nil
			}
			masked := *channel
...
func newNotificationsUpdateCommand(state *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if isTTYRich(state) {
				fmt.Fprintln(os.Stderr, styles.Success("Updated notification channel "+styles.Bold.Render(styles.SafeText(channel.Name))))
				return nil
			}
			masked := *channel
			masked.Config = maskRawJSON(channel.Config, false)
...
func newNotificationsDeleteCommand(state *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if isTTYRich(state) {
				fmt.Fprintln(os.Stderr, styles.Success("Deleted notification channel "+styles.Bold.Render(styles.SafeText(args[0]))))
				return nil
			}
