...
func newDiagnoseCommand(state *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			checks = append(checks, diagnoseCheck("api key present", state.opts.apiKey != "", boolString(state.opts.apiKey != ""), "run `strait login` or set STRAIT_API_KEY"))
			checks = append(checks, diagnoseCheck("project set", state.opts.projectID != "", state.opts.projectID, "set --project or STRAIT_PROJECT"))
...
func newDiagnoseRunCommand(state *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "strait diagnose run run_123\n  strait diagnose run run_123 --follow\n  strait diagnose run run_123 --level error",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for {
				...
				case "queued", "delayed", "dequeued", "waiting":
					remediation = append(remediation, map[string]any{"issue": "run not started", "action": "check worker availability and queue pressure with `strait top queue`"})
				case "executing":
					remediation = append(remediation, map[string]any{"issue": "run in progress", "action": "follow events with `strait runs logs " + runID + " --follow`"})
				default:
